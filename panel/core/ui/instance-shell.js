...
// cfg: { edition, brandSub?, pages: [page id or { id, href, label, icon }], backLabel, sync (sync pill), history (history button), t, esc }
(function () {
  ...
  function mount(cfg) {
    ...
    const links = cfg.pages.map((p) => {
      ...
      return `<a href="${page.href}" data-page="${page.id}">${page.icon}<span class="side-label">${esc(t(page.label))}</span></a>`;
    }).join('\n      ');
